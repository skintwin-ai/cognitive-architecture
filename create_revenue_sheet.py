...
dashboard = workbook.create_sheet('Dashboard', 0)  # Add at the beginning
...
dashboard['A3'].alignment = Alignment(horizontal='center') # Center align header

dashboard['A15'] = 'Cost Structure'
dashboard['A15'].font = Font(bold=True, size=14)
dashboard.merge_cells('A15:J15')
dashboard['A15'].alignment = Alignment(horizontal='center') # Center align header

dashboard['A29'] = 'Revenue Breakdown' # Adjusted row for spacing
dashboard['A29'].font = Font(bold=True, size=14)
dashboard.merge_cells('A29:J29')
dashboard['A29'].alignment = Alignment(horizontal='center') # Center align header
...
dashboard['A4'] = 'Metric'
dashboard['A4'].font = Font(bold=True, color="FFFFFF")
dashboard['A4'].fill = PatternFill(start_color="4F81BD", end_color="4F81BD", fill_type="solid")
dashboard['A4'].border = thin_border

for i, year in enumerate(['Year 1', 'Year 2', 'Year 3', 'Year 4', 'Year 5']):
    cell = dashboard.cell(row=4, column=i+2)
    cell.value = year
...
# Define fixed row numbers for key metrics in Revenue sheet
revenue_total_row_on_revenue_sheet = last_row + 1
gross_profit_row_on_revenue_sheet = net_income_row_in_profitability - 12  # Based on position in profitability section
gross_margin_row_on_revenue_sheet = net_income_row_in_profitability - 11
ebitda_row_on_revenue_sheet = net_income_row_in_profitability - 8
ebitda_margin_row_on_revenue_sheet = net_income_row_in_profitability - 7
net_income_dashboard_row_on_revenue_sheet = net_income_row_in_profitability
net_margin_row_on_revenue_sheet = net_income_row_in_profitability + 1

for i, metric in enumerate(metrics):
    metric_row_on_dashboard = i + 5
    cell_metric_name = dashboard.cell(row=metric_row_on_dashboard, column=1)
    cell_metric_name.value = metric
    cell_metric_name.font = Font(bold=True)
    cell_metric_name.border = thin_border
    
    for j in range(5):
        year_col_letter = chr(70 + j)  # F, G, H, I, J
        cell_data = dashboard.cell(row=metric_row_on_dashboard, column=j+2)
        
        if metric == 'Total Revenue':
            cell_data.value = f'=Revenue!{year_col_letter}{revenue_total_row_on_revenue_sheet}'
        elif metric == 'Gross Profit':
            cell_data.value = f'=Revenue!{year_col_letter}{gross_profit_row_on_revenue_sheet}'
        elif metric == 'Gross Margin %':
            cell_data.value = f'=Revenue!{year_col_letter}{gross_margin_row_on_revenue_sheet}'
            cell_data.number_format = '0.0%'
        elif metric == 'EBITDA':
            cell_data.value = f'=Revenue!{year_col_letter}{ebitda_row_on_revenue_sheet}'
        elif metric == 'EBITDA Margin %':
            cell_data.value = f'=Revenue!{year_col_letter}{ebitda_margin_row_on_revenue_sheet}'
            cell_data.number_format = '0.0%'
        elif metric == 'Net Income':
            cell_data.value = f'=Revenue!{year_col_letter}{net_income_dashboard_row_on_revenue_sheet}'
        elif metric == 'Net Profit Margin %':
            cell_data.value = f'=Revenue!{year_col_letter}{net_margin_row_on_revenue_sheet}'
            cell_data.number_format = '0.0%'
        elif metric == 'COGS as % of Revenue':
            cell_data.value = f'=COGS!{year_col_letter}{cogs_total_row}/Revenue!{year_col_letter}{revenue_total_row_on_revenue_sheet}'
            cell_data.number_format = '0.0%'
        elif metric == 'SG&A as % of Revenue':
            cell_data.value = f'=\'SG&A\'!{year_col_letter}{sga_total_row}/Revenue!{year_col_letter}{revenue_total_row_on_revenue_sheet}'
            cell_data.number_format = '0.0%'
        elif metric == 'Operational Costs as % of Revenue':
            cell_data.value = f'=\'Operational Costs\'!{year_col_letter}{operational_costs_total_row}/Revenue!{year_col_letter}{revenue_total_row_on_revenue_sheet}'
            cell_data.number_format = '0.0%'
        ...
        cell_data.alignment = Alignment(horizontal='right')

# Add cost structure breakdown for Year 1
cost_categories = [
    'Raw Materials', 
    'Packaging Materials', 
    'Direct Labor', 
...
cost_structure_start_row = 16
dashboard.cell(row=cost_structure_start_row, column=1).value = 'Cost Category'
dashboard.cell(row=cost_structure_start_row, column=1).font = Font(bold=True, color="FFFFFF")
dashboard.cell(row=cost_structure_start_row, column=1).fill = PatternFill(start_color="4F81BD", end_color="4F81BD", fill_type="solid")
dashboard.cell(row=cost_structure_start_row, column=1).border = thin_border

dashboard.cell(row=cost_structure_start_row, column=2).value = 'Year 1 Amount'
dashboard.cell(row=cost_structure_start_row, column=2).font = Font(bold=True, color="FFFFFF")
dashboard.cell(row=cost_structure_start_row, column=2).fill = PatternFill(start_color="4F81BD", end_color="4F81BD", fill_type="solid")
dashboard.cell(row=cost_structure_start_row, column=2).alignment = Alignment(horizontal='center')
dashboard.cell(row=cost_structure_start_row, column=2).border = thin_border

dashboard.cell(row=cost_structure_start_row, column=3).value = '% of Total Costs'
dashboard.cell(row=cost_structure_start_row, column=3).font = Font(bold=True, color="FFFFFF")
dashboard.cell(row=cost_structure_start_row, column=3).fill = PatternFill(start_color="4F81BD", end_color="4F81BD", fill_type="solid")
dashboard.cell(row=cost_structure_start_row, column=3).alignment = Alignment(horizontal='center')
dashboard.cell(row=cost_structure_start_row, column=3).border = thin_border

# Add formulas for cost categories
cost_structure_data_start_row = cost_structure_start_row + 1
for i, category in enumerate(cost_categories):
    current_cost_row = cost_structure_data_start_row + i
    # Set the category name
    cell_category_name = dashboard.cell(row=current_cost_row, column=1)
    cell_category_name.value = category
    cell_category_name.font = Font(bold=True)
    cell_category_name.border = thin_border
    
    # Placeholder formulas - these would need to be adjusted based on actual sheet structure
    cell_amount = dashboard.cell(row=current_cost_row, column=2)
    if category in ['Raw Materials', 'Packaging Materials', 'Direct Labor', 'Manufacturing Overhead', 'Contract Manufacturing', 'Quality Control', 'Freight & Logistics (Inbound)']:
        # Link to COGS sheet - Placeholder
        cell_amount.value = '=100000'  
    elif category in ['Facility Costs', 'Equipment Costs', 'Supply Chain Costs']:
        # Link to Operational Costs sheet - Placeholder
        cell_amount.value = '=150000'  
    elif category in ['Sales Expenses', 'Marketing Expenses']:
        # Link to SG&A sheet - Placeholder
        cell_amount.value = '=200000'  
    
    # Calculate percentage
    cell_percentage = dashboard.cell(row=current_cost_row, column=3)
    cell_percentage.value = f'=B{current_cost_row}/SUM(B{cost_structure_data_start_row}:B{cost_structure_data_start_row + len(cost_categories) - 1})'
    cell_percentage.number_format = '0.0%'
    
    # Format cells
    cell_amount.number_format = '#,##0'
    cell_amount.border = thin_border
    cell_amount.alignment = Alignment(horizontal='right')
    cell_percentage.border = thin_border
    cell_percentage.alignment = Alignment(horizontal='right')
...
revenue_breakdown_start_row = 30 # Adjusted row for spacing
dashboard.cell(row=revenue_breakdown_start_row, column=1).value = 'Revenue Category'
dashboard.cell(row=revenue_breakdown_start_row, column=1).font = Font(bold=True, color="FFFFFF")
dashboard.cell(row=revenue_breakdown_start_row, column=1).fill = PatternFill(start_color="4F81BD", end_color="4F81BD", fill_type="solid")
dashboard.cell(row=revenue_breakdown_start_row, column=1).border = thin_border

dashboard.cell(row=revenue_breakdown_start_row, column=2).value = 'Year 1 Amount'
dashboard.cell(row=revenue_breakdown_start_row, column=2).font = Font(bold=True, color="FFFFFF")
dashboard.cell(row=revenue_breakdown_start_row, column=2).fill = PatternFill(start_color="4F81BD", end_color="4F81BD", fill_type="solid")
dashboard.cell(row=revenue_breakdown_start_row, column=2).alignment = Alignment(horizontal='center')
dashboard.cell(row=revenue_breakdown_start_row, column=2).border = thin_border

dashboard.cell(row=revenue_breakdown_start_row, column=3).value = '% of Total Revenue'
dashboard.cell(row=revenue_breakdown_start_row, column=3).font = Font(bold=True, color="FFFFFF")
dashboard.cell(row=revenue_breakdown_start_row, column=3).fill = PatternFill(start_color="4F81BD", end_color="4F81BD", fill_type="solid")
dashboard.cell(row=revenue_breakdown_start_row, column=3).alignment = Alignment(horizontal='center')
dashboard.cell(row=revenue_breakdown_start_row, column=3).border = thin_border

# Add formulas for revenue categories
revenue_breakdown_data_start_row = revenue_breakdown_start_row + 1
for i, category in enumerate(revenue_categories):
    current_revenue_row = revenue_breakdown_data_start_row + i
    # Set the category name
    cell_category_name = dashboard.cell(row=current_revenue_row, column=1)
    cell_category_name.value = category
    cell_category_name.font = Font(bold=True)
    cell_category_name.border = thin_border
    
    # Link to Revenue sheet
    cell_amount = dashboard.cell(row=current_revenue_row, column=2)
    revenue_sheet_row = revenue_data['Revenue Stream'].index(category) + 3 # Find the row in the original data
    cell_amount.value = f'=Revenue!F{revenue_sheet_row}'
    
    # Calculate percentage
    cell_percentage = dashboard.cell(row=current_revenue_row, column=3)
    cell_percentage.value = f'=B{current_revenue_row}/Revenue!F{revenue_total_row_on_revenue_sheet}'
    cell_percentage.number_format = '0.0%'
